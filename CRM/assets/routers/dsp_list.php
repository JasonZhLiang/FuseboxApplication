...
$sql = "SELECT
            ...
            WHERE rou.DeleteFlag = :listSwitch
            ORDER BY  " . $orderBy . "";
$PDOdb->prepare($sql);
...
<?php } else { ?>
    <table class="table table-hover">
        ...
        <tr>
            <th width="50">Edit</th>
            <th width="130">
                <a href="<?= (APP_URL . $XFA["list"]); ?>&order_by=ChassisSN<?= QS_DEBUG ?>">
                    ChassisSN</a>
            </th>
            <th width="130">
            ...
            <th width="125">
                <a href="<?= (APP_URL . $XFA["list"]); ?>&order_by=RouterStatus<?= QS_DEBUG ?>">
                    Status</a>
            </th>
            <th width="160">
                Enter by
        ...
        <?php foreach ($models as $model) { ?>
            <tr valign="top">
                <td align="center">
                    <a href="<?=  APP_URL . $XFA['edit'] . '&id=' . $model['Router_ID'] .'&v='. $model['vHash'] ?>"
            ...
            <tr>
                <td align="center">
                    <a href="<?= APP_URL . $XFA['list'] ?>&show_delete=1" class="btn btn-xs btn-success">
                        <i class="fa-eraser fa ace-icon"></i>Show Deleted Routers</a>
                </td>
            </tr>
            <?php } else { ?>
            <tr>
                <td align="center">
                    <a href="<?= APP_URL . $XFA['list'] ?>&show_delete=0" class="btn btn-xs btn-success">
                        <i class="fa-reply fa ace-icon"></i>Back to available Routers</a>
                </td>
            </tr>
            <?php } ?>
        </table>
    ...
    $(function () {
        $(".js_deleteBtn").on("click", function () {
            ...
            $("#dialog-confirm")
                ...
                .dialog({
                    modal: true,
                    title: "<div class='widget-header widget-header-small'><h4 class='smaller'><i class='fa fa-check ace-icon'></i> Delete Router?</h4></div>",
                    title_html: true,
                    buttons: [
                        {
